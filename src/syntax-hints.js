...
const JS_KEYWORDS = [
  ...
  item('return', 'kw', 'return <giá trị>', 'Trả giá trị về'),
  item('if', 'kw', 'if (dk) { }', 'Rẽ nhánh', 'if ($|) {\n  \n}'),
  item('else', 'kw', 'else { }', 'Nhánh còn lại', 'else {\n  $|\n}'),
  item('for', 'kw', 'for (let i = 0; i < n; i++)', 'Vòng lặp theo chỉ số', 'for (let i = 0; i < $|; i++) {\n  \n}'),
  item('while', 'kw', 'while (dk) { }', 'Lặp khi điều kiện đúng', 'while ($|) {\n  \n}'),
...
const JS_BUILTINS = [
  item('console.log', 'fn', 'console.log(...)', 'In ra để gỡ lỗi', 'console.log($|)'),
  ...
  item('Math.floor', 'fn', 'Math.floor(x)', 'Làm tròn xuống — chia nguyên', 'Math.floor($|)'),
  item('Math.abs', 'fn', 'Math.abs(x)', 'Trị tuyệt đối', 'Math.abs($|)'),
  item('Infinity', 'fn', 'Infinity | -Infinity', 'Vô cực — khởi tạo cho bài tìm min/max'),
...
const JS_MEMBERS = [
  item('push', 'm', 'arr.push(x)', 'Thêm vào cuối — O(1)', 'push($|)'),
  item('pop', 'm', 'arr.pop()', 'Lấy khỏi cuối — O(1)', 'pop()'),
  item('shift', 'm', 'arr.shift()', 'Lấy khỏi đầu — O(n), tránh trong vòng lặp lớn', 'shift()'),
  item('unshift', 'm', 'arr.unshift(x)', 'Thêm vào đầu — O(n)', 'unshift($|)'),
  item('slice', 'm', 'arr.slice(start, end)', 'Cắt ra mảng mới', 'slice($|)'),
  item('splice', 'm', 'arr.splice(i, n)', 'Xoá/chèn tại chỗ', 'splice($|)'),
  item('map', 'm', 'arr.map(x => ...)', 'Biến đổi từng phần tử', 'map(($|) => )'),
  item('filter', 'm', 'arr.filter(x => dk)', 'Lọc phần tử', 'filter(($|) => )'),
  item('reduce', 'm', 'arr.reduce((acc, x) => ..., khởi_tạo)', 'Gộp mảng thành một giá trị', 'reduce((acc, x) => $|, 0)'),
  item('sort', 'm', 'arr.sort((a, b) => a - b)', 'Mặc định sắp theo CHUỖI — luôn truyền hàm so sánh cho số', 'sort((a, b) => a - b)'),
  item('has', 'm', 'map.has(k) | set.has(x)', 'Kiểm tra tồn tại — O(1)', 'has($|)'),
  item('get', 'm', 'map.get(k)', 'Lấy giá trị theo khoá', 'get($|)'),
  item('set', 'm', 'map.set(k, v)', 'Đặt giá trị', 'set($|)'),
  item('add', 'm', 'set.add(x)', 'Thêm vào Set', 'add($|)'),
  item('includes', 'm', 'arr.includes(x)', 'Có chứa? — O(n), Set nhanh hơn', 'includes($|)'),
  item('join', 'm', 'arr.join("")', 'Nối mảng thành chuỗi', 'join($|)'),
